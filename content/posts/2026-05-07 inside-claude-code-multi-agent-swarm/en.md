...
Concurrency here is the same read/write discipline from the [tool execution layer](/en/posts/inside-claude-code-read-write-concurrency-separation), lifted up to the agent level: _"Parallelism is your superpower. Workers are async."_ Read-only research fans out freely; write-heavy implementation runs one worker at a time per set of files. And `SendMessage` lets the coordinator continue a worker whose task is done to reuse its already-warm context, rather than paying to spin up a fresh one.
