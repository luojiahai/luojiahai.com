...
- `src/lib/dictionaries/`: i18n strings (`en.ts` is the canonical shape,
  ...
  UI labels — `works`, `social`, `use` (the whole gear list) and the
  per-aircraft `fly` summaries all live here, once per language. The about
  page's prose is the exception: it lives in `content/pages/about/`.
...
- `content/`: source content. Posts in `content/posts/YYYY-MM-DD title/`;
  ...
  through the same markdown pipeline as posts.
