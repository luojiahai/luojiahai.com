...
- `src/routes/`: SvelteKit routes with `[lang=lang]/` i18n prefix. The blog
  ...
  `about/` (prose from `content/pages/about/`).
...
- `src/lib/dictionaries/`: i18n strings (`en.ts` is the canonical shape,
  ...
  UI labels — `social` and the per-aircraft `fly` summaries still live here,
  once per language. Everything else has moved: the about page's prose, the
  project list, and the gear list all live in `content/`.
...
- `src/lib/content.ts`: typed access to Velite output (posts, pages,
  categories, projects, use). Collections whose fields are localized inline expose
...
- `content/`: source content. Posts in `content/posts/YYYY-MM-DD title/`;
  ...
  `content/projects/projects.yml`, `content/use/use.yml` — where array order
  is display order and only the fields that actually vary carry an inline
  `{ en, zh }`.
