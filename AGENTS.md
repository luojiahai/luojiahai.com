...
- `src/lib/dictionaries/`: i18n strings (`en.ts` is the canonical shape,
  ...
  UI labels — `social`, `use` (the whole gear list) and the per-aircraft
  `fly` summaries all live here, once per language. The about page's prose
  and the project list are the exceptions: they live in `content/`.
...
- `src/lib/content.ts`: typed access to Velite output (posts, pages,
  categories, projects). Collections whose fields are localized inline expose
...
- `content/`: source content. Posts in `content/posts/YYYY-MM-DD title/`;
  ...
  `content/projects/projects.yml` — where array order is display order and
  only the fields that actually vary carry an inline `{ en, zh }`.
